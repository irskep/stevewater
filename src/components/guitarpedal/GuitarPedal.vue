  ...
  <div class="GuitarPedal" v-bind:id="pedalID">
    ...
    <div class="ThreeSectionStompbox">
      <div class="Controls">
        ...
        <div v-if="knobConfigName === 'NByTwo'" class="Knobs__StraightRow">
          <SomeControl
            ...
            v-bind:label="pedalWords.knobLabels[numAcross + n - 1]"
          ></SomeControl>
        </div>
        ...
        <div
          v-if="knobConfigName === 'TriangleDown'"
          class="Knobs__TriangleDown"
        >
          <SomeControl
            v-for="n in 3"
            v-bind:key="n"
      ...
      <Ports
        ...
        v-bind:outputLabels="pedalWords.outputLabels"
      ></Ports>
...
const flexJustifyContent = ["center", "space-between"];

const flexDirectionRow = ["row", "row-reverse"];
...
const footConfigs = ["Pusher", "Pusher", "OneSwitch", "MultiSwitch"];

const footSwitchStyles = ["hex", "circle"];
...
const bg2TextColors = ["#fff", "#000", "var(--textOnBg)"];
...
export default {
  ...
  computed: {
    ...
    pedalWords: function() {
      ...
      const w = {
        knobLabels: [...Array(20)].map(() =>
          pedalWordsGen.gen("knobLabel", model)
        ),
        switchLabels: [...Array(5)].map(() =>
          pedalWordsGen.gen("switchLabel", model)
        ),
    ...
    numFootSwitches: function() {
      return choiceItem(this.getRandom(), [2, 2, 2, 3, 3, 3, 4]);
    },
    ...
    shouldRotateLogo: function() {
      return this.getRandom() < 0.3;
    },
    nameLayoutRandomNumber: function() {
      return this.getRandom();
    },
      ...
      return `
        <style>
          .GuitarPedal#${this.pedalID} {
            ...
            --name-flex-justify-content: ${choiceItem(
              this.getRandom(),
              flexJustifyContent
            )};
            --name-flex-direction: 'row';
            --name2-flex-justify-content: ${choiceItem(
              this.getRandom(),
              flexJustifyContent
            )};
            --name2-flex-direction: ${choiceItem(
              this.getRandom(),
              flexDirectionRow
            )};
          }
